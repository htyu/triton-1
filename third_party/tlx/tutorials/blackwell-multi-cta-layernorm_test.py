...
kernel_configs_multi_cta = [
    triton.Config(
        {"BLOCK_SIZE_M": m, "num_reduction_ctas": ctas},
        num_warps=nw,
        ctas_per_cga=(1, ctas, 1),
    ) for m in [1, 2] for nw in [4, 8, 16, 32] for ctas in [1, 2]
]
...
def prune_invalid_configs(configs, named_args, **kwargs):
    ...
    for conf in configs:
        ...
        N = kwargs["N"]
        # if rounding up reduces num_ctas then don't
        # use this config. Otherwise tail ctas won't have
        # any work
        blocksize_n_rounded = triton.next_power_of_2(N // num_ctas)
        if triton.cdiv(N, blocksize_n_rounded) != num_ctas:
            continue
        # cp.async does not support transfers smaller than 4 bytes per thread
        # Use element_size from X tensor if available, otherwise assume float16 (2 bytes)
        element_size = 2  # default to float16
        for key in named_args:
            tensor = named_args[key]
            if hasattr(tensor, "element_size"):
                element_size = tensor.element_size()
                break
        block_size_m = conf.kwargs.get("BLOCK_SIZE_M")
        num_threads = conf.num_warps * 32
        bytes_per_thread = (block_size_m * blocksize_n_rounded * element_size) // num_threads
...
@triton.heuristics({
    "BLOCK_SIZE_N": lambda args: triton.next_power_of_2(args["N"] // args["num_reduction_ctas"]),
    "SHOULD_MASK_ROW": lambda args: args["M"] % args["BLOCK_SIZE_M"] != 0,
    "SHOULD_MASK_COL": lambda args: args["N"] % args["BLOCK_SIZE_N"] != 0,
})
@triton.jit
...
@pytest.mark.parametrize("M,N", [(4, 16384), (4, 32768), (4, 65536), (1152, 16384), (1152, 32768), (1152, 65536)])
